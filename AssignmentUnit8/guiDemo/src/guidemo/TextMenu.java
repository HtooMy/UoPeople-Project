...
public class TextMenu extends JMenu {
	...
	public TextMenu(DrawPanel owner) {
		...
		lineSpacing.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				String s = JOptionPane.showInputDialog(panel, "Multiply the default line spacing by what amouont?",
						lineSpace);
				if (s != null && s.trim().length() > 0) {
					try {
						double newLineSpace = Double.parseDouble(s.trim());
						panel.getTextItem().setLineHeightMultiplier(newLineSpace);
						panel.repaint();
	...
	/*
	 * This method contains a menu which contains three radio buttons
	 * This method can justify the menu
	 */
